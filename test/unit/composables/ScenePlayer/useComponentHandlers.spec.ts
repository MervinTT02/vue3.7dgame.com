...
describe("applyComponents", () => {
  ...
  describe("Rotate component", () => {
    it("honors disabled startup and supports idempotent start/stop", () => {
      const ctx = makeCtx();
      const result = applyComponents({
        mesh: makeMesh(), uuid: "u1", ctx,
        components: [{ type: "Rotate", parameters: {
          uuid: "r1", speed: { x: 0, y: 15, z: 0 }, isRotating: false,
        }}],
      });
      const setRotating = (result.data as { setRotating: (enabled: boolean) => void }).setRotating;
