...
const requestEditor = (
  ...
  new Promise<Record<string, unknown>>((resolve, reject) => {
    ...
    if (!requestId) {
      reject(new Error("实体编辑器尚未准备完成"));
      return;
    }

    const timer = window.setTimeout(() => {
      pendingRequests.delete(requestId);
      reject(new Error(`实体编辑器请求超时：${action}`));
    }, timeoutMs);
...
const saveWebMcpMutation = async (
  response: Record<string, unknown>,
  failureMessage: string
) => {
  if (response.noChange) return;
...
onMounted(() => {
  ...
  webMcpLifecycle = registerEntityEditorWebMcpTools({
    getContext: () => ({
