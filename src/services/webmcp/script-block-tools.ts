...
import type { ScriptWorkspaceSummary } from "./meta-script-tools";

type JsonRecord = Record<string, unknown>;

export type ScriptOwnerKind = "entity" | "scene";

export type ScriptBlockBatchPreview = {
...
const cloneOperations = (value: unknown) => {
  ...
  if (serialized.length > MAX_OPERATIONS_BYTES) {
      ...
      async execute(input) {
        ...
        if (!draft) {
          return {
            ...
            draftId,
            message: "积木批量修改草稿不存在或已经过期，请重新预览",
          };
        }
        const context = options.getContext();
        if (
          context.ownerKind !== draft.preview.ownerKind ||
          context.ownerId !== draft.preview.ownerId
        ) {
          drafts.delete(draftId);
          return {
            status: "owner_changed",
            draftId,
            message: "当前脚本页面已经切换，请重新预览积木修改",
          };
        }

        const confirmed = await options.confirmBlockBatch(draft.preview);
        if (!confirmed) {
          drafts.delete(draftId);
