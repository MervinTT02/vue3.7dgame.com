...
export const createEntityComponentTools = (
  ...
  const drafts = new Map<string, ComponentBatchDraft>();
      ...
      async execute(input) {
        ...
        if (options.getEntityId() !== draft.preview.entityId) {
          ...
          return {
            ...
            draftId,
            message: "当前实体已切换，请重新预览组件修改",
          };
        }

        const confirmed = await options.confirmComponentBatch(draft.preview);
        if (!confirmed) {
          drafts.delete(draftId);
