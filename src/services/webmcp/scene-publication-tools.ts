...
export const createScenePublicationTools = (
  ...
  const drafts = new Map<string, ScenePublicationDraft>();
      ...
      async execute(input) {
        ...
        if (options.getSceneId() !== draft.preview.sceneId) {
          ...
          return {
            ...
            draftId,
            message: "当前场景已切换，请在新场景中重新检查",
          };
        }

        // Consume before awaiting UI confirmation: each draft is single-use.
        drafts.delete(draftId);
        const confirmed = await options.confirmScenePublication(draft.preview);
        if (!confirmed) {
          drafts.delete(draftId);
