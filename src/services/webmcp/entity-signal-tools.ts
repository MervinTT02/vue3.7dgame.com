...
export const createEntitySignalTools = (
  ...
  const drafts = new Map<string, SignalBatchDraft>();
      ...
      async execute(input) {
        ...
        if (options.getEntityId() !== draft.preview.entityId) {
          ...
          return {
            ...
            draftId,
            message: "当前实体已切换，请重新预览信号修改",
          };
        }

        const confirmed = await options.confirmSignalBatch(draft.preview);
        if (!confirmed) {
          drafts.delete(draftId);
