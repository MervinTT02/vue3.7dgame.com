...
export const createEntityAssetLifecycleTools = (
  ...
  const drafts = new Map<string, AssetRenameDraft>();
  ...
  return [
    {
      ...
      execute(input) {
        ...
        return buildEntityAssetUsage(options.getEntity(), {
          resourceId,
          maxReferences,
        });
      },
      ...
      async execute(input) {
        ...
        if (!draft) {
          return {
            ...
            draftId,
            message: "素材重命名草稿不存在或已经过期，请重新预览",
          };
        }
        if (options.getEntity()?.id !== draft.preview.entityId) {
          drafts.delete(draftId);
          return {
            status: "entity_changed",
            draftId,
            message: "当前实体已切换，请重新预览素材重命名",
          };
        }

        const confirmed = await options.confirmAssetRename(draft.preview);
        if (!confirmed) {
          drafts.delete(draftId);
